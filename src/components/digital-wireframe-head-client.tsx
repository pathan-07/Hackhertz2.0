...
const WireframeHeadCanvas = () => {
  ...
  useEffect(() => {
    ...
    const drawCircuitPatterns = (ctx: CanvasRenderingContext2D, centerX: number, centerY: number, time: number) => {
      ...
      // Draw circuit lines across the face - use deterministic patterns instead of Math.random()
      for (let i = 0; i < 15; i++) {
        // Use deterministic values based on i and time
        const startAngle = ((i / 15) * Math.PI * 2) + (time * 0.1);
        const endAngle = startAngle + Math.PI * (0.5 + ((i % 5) / 5) * 0.5);
        const radius = 60 + ((i % 10) / 10) * 60;
...
export function DigitalWireframeHead() {
  return (
    <ClientOnly>
      <WireframeHeadCanvas />
    </ClientOnly>
